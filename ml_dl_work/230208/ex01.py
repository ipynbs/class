...
from sklearn.model_selection import train_test_split  # 자동으로 나눠짐
...
# (원값 - 평균값)/표준점수
fish_scaled = (fish_data - np.mean(fish_data, axis=0)) / \
    np.std(fish_data, axis=0)
print(fish_scaled[:5])

new = (np.array([25, 150]) - np.mean(fish_data, axis=0)) / \
    np.std(fish_data, axis=0)
print(new)
